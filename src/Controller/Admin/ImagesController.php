...
class ImagesController extends Controller
{
    ...
    /**
     * @Route("/create/", name="admin-images-create")
    ...
    public function create(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            return $this->redirectToRoute('admin-images-create-type', ['type' => $form->get('value')->getNormData()]);
        }
    ...
    /**
     * @Route("/create/{type}/", name="admin-images-create-type", defaults={"type":""}, requirements={"type": "[a-z]+"})
     * @param \Symfony\Component\HttpFoundation\Request                                    $request
     *
     * @param \Symfony\Component\Security\Core\Authorization\AuthorizationCheckerInterface $authorizationChecker
     *
     ...
     * @throws \Symfony\Component\Security\Core\Exception\AccessDeniedException
     */
    public function createType(Request $request, AuthorizationCheckerInterface $authorizationChecker)
    {
        if (false === $authorizationChecker->isGranted('ROLE_ADMIN')) {
            throw new AccessDeniedException('Unable to access this page!');
        }
     ...
     * @param string       $fileName Thumbnails will use this as a directory followed by a child directory lg, md, sm,
     *                               xs
     ...
     * @throws \Psr\Cache\InvalidArgumentException
     */
    private function sendImageToAWS(string $imageType, UploadedFile $file, string $fileName, string $fileNameExt): void
    {
        switch ($imageType) {
            case 'panoramic':
                $dirPrefix = 'pano';
